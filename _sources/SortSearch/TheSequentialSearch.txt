...
Реализация этого алгоритма на Python показана в :ref:`CodeLens 1 <lst_seqsearchpython>`. Функция требует список и элемент, который мы ищем, а возвращает логическое значение, говорящее о его присутствии. Булева переменная ``found`` инициализируется значением ``False``, и если элемент обнаруживаем в списке, то ей присваивается ``True``.
